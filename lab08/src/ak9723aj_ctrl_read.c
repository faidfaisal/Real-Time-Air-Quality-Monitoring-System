 ...
 * AK9723AJ sensor into a packed struct, demonstrating sequential single-byte
 * reads over I²C with repeated START conditions.
 */
...
{
    TWI0_AK9723_init();
    volatile uint8_t *cntl_ptr = (volatile uint8_t *)&default_cntl;

    while (1)
    {
        for (uint8_t i = 0; i < 10; i++)
            cntl_ptr[i] = TWI0_AK9723_byte_read(AK9723AJ_ADDR, 0x0F + i);
    }
}
